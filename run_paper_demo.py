...
from simple_trader.paper import PaperTradingEngine
...
def main():
    ...
    for fill in result.fills:
        report = fill.report
        line = (
            f"{fill.date} {fill.source} {report.symbol} {report.action} "
            f"size={report.size} price={report.price:.2f} fee={report.fee:.2f}"
        )
